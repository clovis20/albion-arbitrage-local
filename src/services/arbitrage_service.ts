...
export class ArbitrageService {
  ...
  public async processMarketData(data: MarketData): Promise<void> {
    ...
    try {
      ...
      // Remover: await this.redis.incrementCounter("market_updates_processed", 3600)
    } catch (error) {
  ...
  private async processOrderGroup(
    ...
    // Remover: await this.redis.cacheMarketPrice(
  ...
  private async calculateArbitrageOpportunities(): Promise<void> {
    ...
    // Remover: await this.redis.cacheArbitrageOpportunities(opportunities, 60 * 2)
    // this.io
  ...
  public async getTopOpportunities(limit: number = 50): Promise<any[]> {
    // Remover: const cached = await this.redis.getCachedArbitrageOpportunities()
  ...
  public async getWeeklyAverages(): Promise<any[]> {
    // Remover: const cached = await this.redis.get("weekly_averages")
    // if (cached && Array.isArray(cached)) return cached

    const averages = await this.db.getWeeklyAverages()
    // Remover: await this.redis.set("weekly_averages", averages, 3600)
